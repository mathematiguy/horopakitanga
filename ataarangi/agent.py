class RationalSpeechAgent:
    ...
    def __init__(self, world_states, utterances, literal_listener_matrix, prior=None):
        ...
        self.prior = (
            ...
            else np.ones(len(world_states)) / len(world_states)
        )
    ...
    def pragmatic_speaker(self, world_state_index, alpha=1.0):
        utilities = np.array(
            [
                safe_log(alpha * probability)
                for probability in self.literal_listener_matrix[world_state_index, :]
            ]
        )
    ...
    def pragmatic_listener(self, utterance_index):
        speaker_matrix = np.array(
            [self.pragmatic_speaker(ws) for ws in range(len(self.world_states))]
        )
...
class TeacherAgent(RationalSpeechAgent):
    ...
    def __init__(
        ...
        student_model_matrix,
        prior=None,
    ):
        super().__init__(world_states, utterances, literal_listener_matrix, prior)
        self.student_model_matrix = normalize(
            np.array(student_model_matrix)
        )  # Teacher's belief about the student's knowledge
    ...
    def suggest_world_state(self):
        ...
        valid_indices = [
            index
            for index, row in enumerate(self.student_model_matrix)
            if not np.any(row == 0)
        ]
        ...
        student_entropy_values = np.array(
            [
                (
                    entropy(self.student_model_matrix[state_index, :])
                    if state_index in valid_indices
                    else -np.inf
                )
                for state_index in range(len(self.world_states))
            ]
        )
        teacher_entropy_values = np.array(
            [
                (
                    entropy(self.literal_listener_matrix[state_index, :])
                    if state_index in valid_indices
                    else -np.inf
                )
                for state_index in range(len(self.world_states))
...
class StudentAgent(RationalSpeechAgent):
    ...
    def update_known_utterances(self):
        ...
        for utterance_index in range(len(self.utterances)):
            if np.any(
                np.sum(self.literal_listener_matrix[:, utterance_index] != 0) == 1
            ):
    ...
    def suggest_world_state(self):
        ...
        valid_indices = [
            index
            for index, row in enumerate(self.literal_listener_matrix)
            if index not in self.attempt_history
            or not self.attempt_history[index]
            and not np.any(row == 0)
            and all(
                utterance_index in self.known_utterances
                for utterance_index, val in enumerate(row)
                if val > 0
            )
        ]
...
teacher = TeacherAgent(
    ...
    student_model_matrix=np.full(
        (len(world_states), len(utterances)), 1 / len(world_states)
    ),  # Teacher's initial model of the student's knowledge
)

student = StudentAgent(
    world_states=world_states,
    utterances=utterances,
    literal_listener_matrix=np.full(
        (len(world_states), len(utterances)), 1 / len(world_states)
    ),  # Student starts with no specific knowledge
)
