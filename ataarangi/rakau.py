...
class WorldState:
    ...
    def toggle_random_selections(self):
        ...
        selected_indices = random.sample(
            range(len(self.ngā_rākau)), num_selections
        )  # Get n unique random indices
        for index in selected_indices:
            self.ngā_rākau[
                index
            ].toggle_selection()  # Toggle the selection of each chosen stick
        ...
        while True:
            location = random.choice(
                [
                    x
                    for x in range(1, 21)
                    if not x in [rākau.location for rākau in self.ngā_rākau]
                ]
            )
            new_rākau = Rākau(
                random.choice(
            ...
            logging.info(
                f"Added {new_rākau}. Current complexity (entropy): {current_complexity:.2f}"
            )
    ...
    def calculate_entropy(self):
        ...
        color_entropy = -sum(
            (
                p / len(self.ngā_rākau) * np.log2(p / len(self.ngā_rākau))
                for p in self.colors.values()
            )
        )
        height_entropy = -sum(
            (
                p / len(self.ngā_rākau) * np.log2(p / len(self.ngā_rākau))
                for p in self.heights.values()
            )
        )
        x_entropy = -sum(
            (
                p / len(self.ngā_rākau) * np.log2(p / len(self.ngā_rākau))
                for p in self.locations.values()
            )
        )
    ...
    def draw(self):
        ...
        for rākau in self.ngā_rākau:
            ...
            linestyle = (
                "--" if rākau.selected else "-"
            )  # Dashed if selected, solid otherwise
            linewidth = (
                2 if rākau.selected else 1
            )  # Slightly thicker if selected, else normal border
            ax.bar(
                rākau.location,
                rākau.height,
                color=rākau.color,
